    ...
    try {
        ...
        const [dayIpvAlarms] = await connection.execute(`
            SELECT
                ...
                pc.num_strings, -- <<< ADIÇÃO IMPORTANTE: Agora pegamos o número de strings
            ...
            WHERE
                ...
                AND sd.status <> -1 -- ADIÇÃO: Ignorar inversores offline
                AND (
        ...
        // Este loop 'forEach' processa a array dayIpvAlarms UMA ÚNICA VEZ antes do loop principal.
        dayIpvAlarms.forEach(detection => {
            // Converte todas as propriedades ipv para números flutuantes
            // O loop agora usa detection.num_strings como limite
            for (let i = 1; i <= detection.num_strings; i++) { // <<< CORRIGIDO
                const ipvKey = `ipv${i}`;
                detection[ipvKey] = parseFloat(detection[ipvKey] || 0);
            }
            // Converte a propriedade greatest_ipv_in_group para número flutuante
            detection.greatest_ipv_in_group = parseFloat(detection.greatest_ipv_in_group || 0);
        });
        // --- Fim do bloco de preparação dos dados ---


        // NOVO: Buscar contagens consecutivas existentes para HALF_STRING_WORKING
        // Este bloco precisa estar ANTES do loop principal de detecção de alarmes.
        const [consecutiveCountsRows] = await connection.execute(
            `SELECT plant_name, inverter_id, consecutive_count
             FROM consecutive_alarm_counts
             WHERE alarm_type = 'HALF_STRING_WORKING'`
        );
        const consecutiveCountsMap = new Map();
        consecutiveCountsRows.forEach(row => {
            const key = `${row.plant_name}_${row.inverter_id}`;
            consecutiveCountsMap.set(key, row.consecutive_count);
        });


        // --- Loop principal de detecção e gerenciamento de alarmes ---
        // Este loop processa CADA 'detection' individualmente, com os dados já preparados.
        for (const detection of dayIpvAlarms) {
            const plantName = detection.plant_name;
            const inverterId = detection.inverter_id;
            const numStrings = detection.num_strings; // Agora disponível aqui!
            const greatestIpv = detection.greatest_ipv_in_group;

            // --- STRING_DOWN Detection ---
            // O loop agora usa detection.num_strings como limite
            for (let i = 1; i <= numStrings; i++) { // <<< CORRIGIDO
                const ipvKey = `ipv${i}`;
                ...
                if (detection[ipvKey] <= 0.5 && greatestIpv > 8.0) {
                    const alarmKey = `${detection.plant_name}_${detection.inverter_id}_${alarmType}_${problemDetails}`;
                    if (!activeAlarmsMap.has(alarmKey)) {
                        const message = `String ${i} do inversor ${detection.inverter_id} da planta ${detection.plant_name} está com produção próxima de zero (${(detection[ipvKey] || 0).toFixed(2)}A) enquanto outras strings estão ativas (pico: ${greatestIpv.toFixed(2)}A).`;
            ...
            if (greatestIpv >= 13.0) {
                ...
                const consecutiveKey = `${detection.plant_name}_${detection.inverter_id}`;
                let consecutiveCount = consecutiveCountsMap.get(consecutiveKey) || 0;

                if (isHalfStringWorking) {
                    consecutiveCount++; // Incrementa a contagem se a condição for met
                    consecutiveCountsMap.set(consecutiveKey, consecutiveCount); // Atualiza no mapa

                    if (consecutiveCount >= 4) { // Só dispara o alarme se a contagem for >= 4
                        ...
                        if (!activeAlarmsMap.has(alarmKey)) {
                            const message = `String ${halfWorkingStringNum} do inversor ${detection.inverter_id} da planta ${detection.plant_name} está com produção de ${halfWorkingStringIpv.toFixed(2)}A, o que está entre 30% e 70% da string de maior produção (${greatestIpv.toFixed(2)}A). Isso indica uma série funcionando em paralelo.`;
                            await connection.execute(
                                `INSERT INTO alarms (plant_name, inverter_id, alarm_type, alarm_severity, problem_details, message, triggered_at)
                                 VALUES (?, ?, ?, ?, ?, ?, NOW())`,
                                [detection.plant_name, detection.inverter_id, alarmType, alarmSeverity, problemDetails, message]
                        ...
                        console.log(`[${getFormattedTimestamp()}] HALF_STRING_WORKING detectado para Planta: ${detection.plant_name}, Inversor: ${detection.inverter_id}, String: ${halfWorkingStringNum} (Contagem consecutiva: ${consecutiveCount}/4) - Alarme não disparado ainda.`);
                    }
                } else { // Se a condição HALF_STRING_WORKING NÃO for metida
                    if (consecutiveCount > 0) { // Reseta a contagem apenas se ela era maior que 0
                        console.log(`[${getFormattedTimestamp()}] Resetando contagem consecutiva para HALF_STRING_WORKING para Planta: ${detection.plant_name}, Inversor: ${detection.inverter_id}.`);
                        consecutiveCountsMap.set(consecutiveKey, 0);
                    }
                }
            } else { // Se greatestIpv < 13.0 (Não se qualifica para HALF_STRING_WORKING)
                const consecutiveKey = `${detection.plant_name}_${detection.inverter_id}`;
                if (consecutiveCountsMap.has(consecutiveKey) && consecutiveCountsMap.get(consecutiveKey) > 0) {
                    console.log(`[${getFormattedTimestamp()}] Resetando contagem consecutiva para HALF_STRING_WORKING para Planta: ${detection.plant_name}, Inversor: ${detection.inverter_id} (Pico IPV abaixo do limite).`);
                    consecutiveCountsMap.set(consecutiveKey, 0);
                }
            }
        }
            ...
            if (alarm.alarm_type === 'STRING_DOWN' || alarm.alarm_type === 'HALF_STRING_WORKING') {
                ...
                const [currentInverterData] = await connection.execute(`
                    SELECT
                        ...
                        pc.num_strings, -- <<< ADIÇÃO AQUI TAMBÉM
                        pc.string_grouping_type, -- <<< ADIÇÃO AQUI TAMBÉM
                        sd.status
                    FROM solar_data sd
                    JOIN plant_config pc ON sd.plant_name = pc.plant_name AND sd.inverter_id = pc.inverter_id
                ...
                const currentNumStrings = currentInverterData.length > 0 ? currentInverterData[0].num_strings : 0; // <<< NOVO
                const currentStringGroupingType = currentInverterData.length > 0 ? currentInverterData[0].string_grouping_type : null; // <<< NOVO
        ...
        // --- Persistir contagens consecutivas atualizadas para HALF_STRING_WORKING ---
        // Este bloco salva as contagens no banco de dados após processar todos os alarmes.
        for (const [key, count] of consecutiveCountsMap.entries()) {
            const [plantName, inverterId] = key.split('_');
            if (count > 0) {
                await connection.execute(
                    `INSERT INTO consecutive_alarm_counts (plant_name, inverter_id, alarm_type, consecutive_count)
                     VALUES (?, ?, 'HALF_STRING_WORKING', ?)
                     ON DUPLICATE KEY UPDATE consecutive_count = VALUES(consecutive_count)`,
                    [plantName, inverterId, count]
                );
            } else {
                ...
                await connection.execute(
                    `DELETE FROM consecutive_alarm_counts
                     WHERE plant_name = ? AND inverter_id = ? AND alarm_type = 'HALF_STRING_WORKING'`,
